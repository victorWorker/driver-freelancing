...
<template>
    <div class="vue-container">
        <div class="container form-regi-slide hero p-0 mobilepadding">
        <div class="hero-height">
             <img :src="`${baseURL}images/van.svg`" class="bg-hero" />
            
           <div class="row ">
                <div class="heroimgholder timeline">
                   <div class="headings">
                    <h3 class="primarylightfont mb-0 topheading uppercase">O SEU PEDIDO NA
</h3>
                    <h3 class="primaryheavyfont uppercase">BUSCARGO EM 3 PASSOS!</h3>

                </div>

                       <div class="circles d-flex justify-content-between position-relative">
                    <div class="circle me-2">
                        <div>
                            <h1 class="primaryheavyfont">1.</h1>
                            <p class="mb-0 uppercase">{{ language.step_1 }}</p>
                        </div>
                    </div>

                    <div class="circle me-2">
                        <div>
                            <h1 class="primaryheavyfont">2.</h1>
                            <p class="mb-0 uppercase">{{ language.step_2 }}</p>
                        </div>
                    </div>

                    <div class="circle me-2">
                        <div>
                            <h1 class="primaryheavyfont">3.</h1>
                            <p class="mb-0 uppercase">{{ language.step_3 }}</p>
                        </div>
                    </div>
                    ...
                    <hr class="linethrough d-none d-sm-block">
                </div>
                        <div class="play-btn">
                            <a data-bs-toggle="modal" data-bs-target="#modalVideo">
                             <i class="fas fa-play"></i>
                            </a>

                            Clique aqui para video demonstrativo
                                                        
                          </div>
                          <div  id="modalVideo" tabindex="-1" aria-labelledby="exampleModalLabel" class="modal fade"  aria-modal="true" role="dialog">
  ...
  <source :src="`${baseURL}images/intro-video.mp4`" type="video/mp4"></video>
                ...
                <div  class="herotextholder mt-5 mt-sm-0">
        <h2  class="primaryheavyfont main-head uppercase">{{ language.hero_form_heading_one }}
</h2> 
<h2  class="primaryheavyfont secondarycolor main-head uppercase">{{ language.hero_form_heading_two }}</h2>
        <hr class="extrabg"> 
         
        <p class="mt-2"> {{ language.hero_form_desc }} </p> 
        <!-- multistep form -->
<form id="msform" v-on:submit.prevent="submitForm">
  <!-- progressbar -->
  
  <!-- fieldsets -->
  <fieldset>
<img :src="`${baseURL}images/selection_new.gif`" class="mb-4 img-fluid" alt="">    
<h2 class="fs-title">{{ language.hero_form_step1 }} </h2>
<label>{{ language.hero_form_step1_whatareyoumoving }}</label>

    <select required name="" id="category" v-model="form.category">
      <option selected="" value="Choose Transport"> Choose Transport </option>
      <option value="1">Mobília</option>
      <option value="3">Itens frágeis</option>
      <option value="4">Mudanças pessoais</option>
      <option value="18">Carros</option>
      <option value="19">Barcos</option>
      <option value="20">Motas</option>
      <option value="21">Animais</option>
      <option value="22">Partes de automóvel</option>
      <option value="23">Caixas</option>
      <option value="24">Arte</option>
      <option value="25">Outros</option>
    </select>
    <label>  {{ language.hero_form_step1_jobtitle }}</label>
    <input required type="text" name="title" placeholder="Job Title" id="title" v-model="form.title" />

    <input type="button" name="next" class="next action-button bg-primary uppercase" value="Próximo" />
  </fieldset>
  <fieldset>
<img :src="`${baseURL}images/pickup_new.gif `" class="mb-4 img-fluid" alt="">    
    <h2 class="fs-title"> {{ language.hero_form_step2 }} </h2>

    <div class="form-inpsec">
    <label> {{ language.hero_form_step2_pickuppoint }} </label>
    <GmapAutocomplete
    ...
    <div class="form-inpsec">
     <label> {{ language.hero_form_step2_dropofpoint }}</label>
    <GmapAutocomplete
    ...
    <input type="button" name="previous" class="previous action-button bg-primary uppercase" value="Anterior" />
    <input type="button" name="next" class="next action-button bg-primary uppercase" value="Próximo" />
  </fieldset>
  <fieldset>
<img :src="`${baseURL}images/deliver_new.gif`" class="mb-4 img-fluid" alt="">    
    <h2 class="fs-title">  {{ language.hero_form_step3 }}</h2>

    <label>  {{ language.hero_form_step3_mail }} </label>
    <input required type="text" name="email" placeholder="E-Mail" id="email" v-model="form.email"  />
    <label> {{ language.hero_form_step3_telephone }} </label>
    <input required type="text" name="phone" placeholder="Telephone" id="phone" v-model="form.phone" />
    <input type="hidden" name="password" id="password" v-model="form.password" />
  ...
  <fieldset>
    <img :src="`${baseURL}images/deliver_new.gif`" class="mb-4 img-fluid" alt="">    
    <h2 class="fs-title">  {{ language.hero_form_step4 }}</h2>

    <label>  {{ language.hero_form_step4_image }} </label>
    <input type="file" name="img" class="img_inpt"  id="img" @change="uploadCImg('#img')" />
    <label> {{ language.hero_form_step4_description }} ({{language.desc_minwords}}) </label>
    <textarea id="description" name="description" minlength="15" rows="4" cols="10" placeholder="Description" v-model="form.description" >
    </textarea>

    <input type="button" name="previous" class="previous action-button bg-primary uppercase" value="Anterior " />
    <input type="submit" id="sbm_btn" class="action-button bg-primary uppercase" value="Submeter" > 
    <div  id="show_msg">Something went worng !!</div>
    <div  id="show_msg_img">Please wait image is being uploaded...</div>
  </fieldset>
               ...
               <!-- <div class="row justify-content-between">
                  <div class="row  flex-xl-row align-items-end">
                     <div class="col-xl-6 top-50 top-xl-0">
                        <div class="promo-tabs tabs horizontal-tabs">
                           <div class="heading bottom-20">
                              <h3 class="heading__title" style="font-size: 30px; color: #FF9224;">GRAB A  CUP OF COFFEE AND DESCRIBE US WHAT YOU WANT TO MOVE</h3>
                           </div>
                           <p class="bottom-0">Buscargo is the choice of thousands of costumers.</p>
                           <a href="#horizontal-tabs__item-1" style="font-size: 12px; color: #FF9224;padding:3%"><span>For Business</span></a>
                                 ...
                                 <div class="horizontal-tabs__item" id="horizontal-tabs__item-1">
                                    <form class="form promo-form promo-form--f5" action="javascript:void(0);" autocomplete="off">
                                       <div class="row offset-20">
                                          <div class="col-md-12">
                                             <select class="form__select form-control" name="transport" v-model="transport">
                                                <option data-display="Choose transport">What are you moving?</option>
                                                <option value="Furniture & Appliances" style="background-image:url(images/sofa.png);width: 25px;">Furniture & Appliances</option>
                                                <option value="Home Removals" style="background-image:url(images/home.png);width: 25px;">Home Removals</option>
                                                <option value="Cars & Vehicles" style="background-image:url(images/car.png);width: 25px;">Cars & Vehicles</option>
                                                <option value="Packaged Items & Parcels" style="background-image:url(images/pack.png);width: 25px;">Packaged Items & Parcels</option>
                                             </select>
                                             <br/>
                                          </div>
                                          <div class="col-sm-6">
                                             <label>
                                                ...
                                                <input class="form__field" type="text" v-model="from" name="from" placeholder="From"/>
                                             </label>
                                          </div>
                                          <div class="col-sm-6">
                                             <label>
                                                ...
                                                <input class="form__field" v-model="to" type="text" name="To" placeholder="To"/>
                                             </label>
                                          </div>
                                       
                                            <div class="col-sm-6 col-md-3">
                                                <a href="form.html" class="button button--green" style="    background: #0d6efd;color: white;font-size: 15px;" @click.prevent="saveDetails">
                                                    <span>Post</span> 
                                                    <svg class="icon">
                     ...
                     <div class="col-xl-6 text-center text-xl-left"><img class="truck-img" src="images/truck.jpg" alt="img">
                        I want to register, but i don ́t have a <a style="color: blue;" href="#">request yet</a>
            ...
            <div class="row justify-content-between features">
                <div class="feature me-2">
                    <i class="fas fa-couch"></i>
                    <div class="uppercase">
                        <h5 class="primaryboldfont mb-0 fhead">{{ language.cat_1_a }}</h5>
                        <p class="mb-0">{{ language.cat_1_b }}</p>
                    </div>
                </div>
                <div class="feature ms-2">
                    <i class="fas fa-car"></i>
                    <div class="uppercase">
                        <h5 class="primaryboldfont mb-0 fhead">{{ language.cat_2_a }}</h5>
                        <p class="mb-0">{{ language.cat_2_b }}</p>
                    </div>
                </div>
                <div class="feature me-2">
                    <i class="fas fa-motorcycle"></i>
                    <div class="uppercase">
                        <h5 class="primaryboldfont mb-0 fhead">{{ language.cat_3_a }}</h5>
                        <p class="mb-0">{{ language.cat_3_b }}</p>
                    </div>
                </div>
                <div class="feature ms-2">
                    <i class="fas fa-home"></i>
                    <div class="uppercase">
                        <h5 class="primaryboldfont mb-0 fhead">{{ language.cat_4_a }}</h5>
                        <p class="mb-0">{{ language.cat_4_b }}</p>
                    </div>
                </div>
                <div class="feature">
                    <i class="fas fa-box"></i>
                    <div class="uppercase">
                        <h5 class="primaryboldfont mb-0 fhead">{{ language.cat_5_a }}</h5>
                        <p class="mb-0">{{ language.cat_5_b }}</p>
                    </div>
                </div>
            </div>
        </div>                 
        ...
        <!---Stats Section-->
        <div class="container-fluid extrabg">
            <div class="container statholder">
                <div class="row justify-content-between">
                    <div class="statbox">
                        <h1 class="primaryheavyfont">100+</h1>
                        <p class="mb-0 primarylightfont">{{ language.stat_1 }}</p>
                    </div>
                    <div class="statbox">
                        <h1 class="primaryheavyfont">1000+</h1>
                        <p class="mb-0 primarylightfont">{{ language.stat_2 }}</p>
                    </div>
                    <div class="statbox">
                        <h1 class="primaryheavyfont">1 TON+</h1>
                        <p class="mb-0 primarylightfont">{{ language.stat_3 }}</p>
                    </div>
                    <div class="statbox">
                        <h1 class="primaryheavyfont">75%</h1>
                        <p class="mb-0 primarylightfont">{{ language.stat_4 }}</p>
                    </div>
                </div>
            </div>
        ...
        <!--TimeLine-->
        <div class="container timeline timeline-new">
            <div class="row">
                <div class="headings mobilepadding">
                    <h1 class="primarylightfont mb-0 topheading uppercase">{{ language.delivered_with }}</h1>
                    <h1 class="primaryheavyfont uppercase">{{ language.in_steps }}</h1>
                </div>
                <div class="circles d-flex justify-content-between position-relative">
                    ...
                    <img :src="`${baseURL}images/banner-img1.jpg`" class="img-fluid" alt=""> 
                     </div>
                      <div class="col-md-4 me-2"> 
                    <img :src="`${baseURL}images/banner-img2.jpg`" class="img-fluid" alt=""> 
                     </div>
                      <div class="col-md-4"> 
                    <img :src="`${baseURL}images/banner-img3.jpg`" class="img-fluid" alt=""> 
                     </div>
        ...
        <!--Information Block-->
            <div class="container-fluid infobox ">
                    <div class="row">
                        <div class="information col-sm-5 offset-sm-1 offset-xxl-2 mobilepadding">
                            <h1 class="primarylightfont mb-0 topheading uppercase">{{ language.several_jobs }}</h1>
                            <h1 class="primaryheavyfont mb-4">{{ language.are_waiting }}</h1>
                            <p class="mb-5">{{ language.several_jobs_desc }}</p>
                            <a href="#" data-bs-toggle="modal" data-bs-target="#exampleModal" class="blackbtn uppercase">{{ language.several_jobs_btn }}</a>
                        </div>
                        <div class="imageblock col-sm-6 col-xxl-5">
                            <img :src="`${baseURL}images/truck_driver-new.png`" class="w-100">
                              <a href="https://buscargo.pt/client/public/index.php/driver_registration" class="blackbtn formoreinfo quotebtn bg-primary uppercase">  {{ language.homepage_driver_btn }}</a>
                        </div>
                    </div>
            </div>
        <!--Information Block-->
        ...
        <!--Delivery Detail box-->
            <div class="container-fluid extrabg delivery">
                <div class="container text-white">
                    <div class="row justify-content-between align-items-center">
                        <div class="info col-sm-7">
                            <h1 class="primarylightfont lineheight2 mb-0 mt-5 mt-sm-0">{{ language.solidary }}</h1>
                            <h1 class="primaryheavyfont">{{ language.deliveries }}</h1>
                            <p>{{ language.solidary_desc }}</p>
                        </div>
                        <div class="freedelivery">
                            <img :src="`${baseURL}images/delivery.png`" v-if="lang === 'en'" class="w-100 mb-5 mb-sm-0" />
                            <img :src="`${baseURL}images/delivery-pt.png`" v-if="lang === 'pt'" class="w-100 mb-5 mb-sm-0" />
                        </div>
                    </div>
        ...
        <!--Information Block-->
        <div class="container-fluid infobox secondarybg text-white ">
            <div class="row align-items-end">
                <div class="imageblock col-sm-6">
                    <img :src="`${baseURL}images/client_ordering.png`" class="w-100 mt-5">
                </div>
                <div class="information col-sm-5 col-xxl-4 mobilepadding">
                    <h1 class="primarylightfont mb-0 topheading uppercase">{{ language.best_shipping }}</h1>
                    <h1 class="primaryheavyfont mb-4 uppercase">{{ language.prices }}</h1>
                    <p class="mb-5">{{ language.best_prices_desc }}</p>
                    <a href="/search"  class="blackbtn uppercase">{{ language.best_prices_btn }}</a>
                </div>
            </div>
        </div>
        <!--Information Block-->
        ...
        <!-- <div class="container-fluid splash">
            <div class="container mobilepadding">
                <div class="row">
                    <h1 class="primarylightfont lineheight2 uppercase">{{ language.client }}</h1>
                    <h1 class="primaryheavyfont mb-5 uppercase">{{ language.feedback }}</h1>
                </div>
                <Testimonials class="mb-5" :language="language" />
            </div>
        </div> -->
        <!--Our Clients-->

     <section class="section testimonials newtestimonil bg--lgray">
                  ...
                  <div class="row">
                     <div class="col-12">
                        <div class="heading heading--white">
                            <span class="heading__pre-title"><font style="vertical-align: inherit;"><font style="vertical-align: inherit;"> {{ language.homepage_testimonials_heading }} </font></font></span>
                           <h3 class="heading__title"><font style="vertical-align: inherit;"><font style="vertical-align: inherit;">{{ language.homepage_testimonials_heading2 }}  </font></font></h3><span class="heading__layout layout--dgray"><font style="vertical-align: inherit;"><font style="vertical-align: inherit;">People</font></font></span>
                        </div>
                     </div>
                  </div>
   ...
   <div class="col-md-4 mb-md-0 mb-5">
      <div class="testimonial">
         <div class="avatar mx-auto">
            <img src="https://buscargo.pt/client/img/testimonials-icons.png" class="rounded-circle testimonials-img z-depth-1 img-fluid">
            </div> 
            <h4 class="font-weight-bold dark-grey-text mt-4">Carminha Araújo</h4>
             <p class="font-weight-normal dark-grey-text">
                <i class="fas fa-quote-left pr-2" aria-hidden="true"></i>
                  I am very satisfied with the work of Buscago, I recommend it.
                 </p> 
                    ...
                    <div class="col-md-4 mb-md-0 mb-5">
                       <div class="testimonial">
                          <div class="avatar mx-auto">
                             <img src="https://buscargo.pt/client/img/testimonials-icons.png" class="rounded-circle z-depth-1 testimonials-img img-fluid"></div>
                              <h4 class="font-weight-bold dark-grey-text mt-4">Isabel</h4> 
                              <p class="font-weight-normal dark-grey-text">
                                 <i class="fas fa-quote-left pr-2" aria-hidden="true"></i> 
                                 Motorista simpático e prestável. As peças chegaram em bom estado. Recomendo.
                                 </p> 
                                  ...
                                  <div class="col-md-4">
                                     <div class="testimonial">
                                        <div class="avatar mx-auto">
                                           <img src="https://buscargo.pt/client/img/testimonials-icons.png" class="rounded-circle z-depth-1 testimonials-img img-fluid"></div> 
                                           <h4 class="font-weight-bold dark-grey-text mt-4">Sonia Cristina</h4> 
                                           <p class="font-weight-normal dark-grey-text">
                                              <i class="fas fa-quote-left pr-2" aria-hidden="true"></i> 
                                           Só tenho a agradecer, correu tudo muito bem, foi uma compra de 8 cadeiras vindas de Braga, para Sesimbra, 5 estrelas o Sr João foi impecável. Custo benefício só tenho a dizer bem. Vou voltar a utilizar.</p> 
            ...
            <section class="partlogos-sec">
<div class="container text-center">
<img :src="`${baseURL}images/logo1.jpg`" class="">
<img :src="`${baseURL}images/logo2.jpg`" class="">
<img :src="`${baseURL}images/logo3.jpg`" class="">
<img :src="`${baseURL}images/logo4.jpg`" class="">
<img :src="`${baseURL}images/logo5.png`" class="">
                    </div>
            </section>
                ...
                <div class="modal-content">
                    <div class="modal-body p-0 m-0">
                        <div class="container-fluid p-0">
                            <div class="row">
                                <div class="headings mb-4 mb-sm-5 px-5 mt-sm-5 mt-4">
                                     <p class="mb-0">
                                         {{ language.homepage_msgmodel_para1 }}
                                        
                                        </p>
                                        <p>
                                            {{ language.homepage_msgmodel_para2 }}
                                       
                                        </p>
                                       <p class="mb-0"> {{ language.homepage_msgmodel_user }} : <span id="usr_name"></span> </p>
                                       <p> {{ language.homepage_msgmodel_pass }} : <span id="usr_pass"></span> </p>
                                   
                                         <div class="btn-continue">  
                                         <button  class="blackbtn formoreinfo quotebtn bg-primary uppercase" @click="chkLogin()" > Continuar</button>
                                         </div>
...
import Testimonials from '../common/Testimonials.vue';
...
export default {
    ...
    data() {
     ...
     methods: {
         ...mapActions("front",["uploadImg", "authLogin"]),
        chkLogin(){
                let loginData = {'email': this.form.email, 'password': this.form.password};
                //let loginData = {'email': 'developerttest@testmail.com', 'password': '34iicg5'};
        ...
        submitForm(){
                this.custData.clocation = {};
                this.custData.dlocation = {};
                var upass = Math.random().toString(20).substring(8);
                this.form.password = upass;
                $('#sbm_btn').prop('disabled', true);
                $('#sbm_btn').val('processing..');
                this.custData.email = this.form.email;
                this.custData.phone = this.form.phone;
                this.custData.title = this.form.title;
                this.custData.collection = this.form.collection;
                this.custData.delivery = this.form.delivery;
                this.custData.delivery = this.form.delivery;
                this.custData.clocation = this.form.geometry.collection.location;
                this.custData.clatitude = this.form.geometry.collection.latitude;
                this.custData.clongitude = this.form.geometry.collection.longitude;
                this.custData.dlocation = this.form.geometry.delivery.location;
                this.custData.dlatitude = this.form.geometry.delivery.latitude;
                this.custData.dlongitude = this.form.geometry.delivery.longitude;
                this.custData.password = upass;
                //this.hasclicked = true;
                
                //this.setforvalue(this.form.email, this.form.phone, this.form.title);
                axios.post('/client/public/index.php/api/getshipment', this.form)
                     .then((res) => {
                      var user_id = res.data.data.user_id;
                      var shipId = res.data.data.id;
                      $('#usr_name').text(this.form.email);
                      $('#usr_pass').text(upass);

                      this.custData.userid = user_id;
                      this.custData.shipid = shipId;
                      this.hasclicked = true;
                      $('#msgModel').modal('show');
...
$(window).on('load', function() {
     $(".pac-container").appendTo(".inp-div");
...
.promo-tabs{width:100%;max-width:770px;position:absolute;bottom:0;left:50%;-webkit-transform:translateX(-50%);-ms-transform:translateX(-50%);transform:translateX(-50%);bottom:-50px;-webkit-box-shadow:0 3px 10px 2px rgba(0,0,0,.04);box-shadow:0 3px 10px 2px rgba(0,0,0,.04);border:none}
.tabs p:last-of-type{margin-bottom:0}
.horizontal-tabs .r-tabs-panel{padding:23px 30px}
