      ...
      <div class="col-lg-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="row px-5">
                        <div class="headingcard mt-5">
                            <h2 class="font-weight-bolder">Latest <span class="font-weight-lighter">Bids</span></h2>
                            <hr>
                        </div>
                        ...
                        <table class="table table-hover">
                            ...
                            <tbody> 
                                <tr v-for="(bid,index) in user.bids" :key="index">
                                    <th>{{index+1}}</th>
                                    <td>{{bid.proposal}}</td>
                                    <td>{{bid.amount}}</td>
                                    <td>{{bid.expiration}}</td>
                                    <td>
                                        <label :class="'badge badge-danger'">{{bid.status}}</label>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
...
export default {
    ...
    methods: {
        ...mapActions("user", ["getTransporter"]),
        async init() {
            let resp = await this.getTransporter(this.$route.params.id);
