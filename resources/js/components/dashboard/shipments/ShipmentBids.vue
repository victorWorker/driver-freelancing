<template>
  <div class="row">
      <div class="col-lg-12 grid-margin stretch-card">
                    ...
                    <table class="table mt-5">
                        ...
                        <tbody>
                            <tr v-for="(bid, index) in bids" :key="index">
                                ...
                                <td> 
                                    <a v-if="bid.status === 'ACTIVE'" href="#" @click.prevent="suspend(bid.id)" class="small">{{language.suspend}} | </a>  
                                    <a v-if="bid.status === 'SUSPENDED'" href="#" @click.prevent="activate(bid.id)" class="small">{{language.activate}} | </a>  
                                    <a href="#" @click.prevent="view(bid)" class="small">{{language.view}} | </a>  
                                    <a href="#" @click.prevent="edit(bid)" class="small">{{language.edit}} | </a>  
                                    <router-link :to='{name:"showdriver",params:{id:bid.driver_id}}' class="small">View driver details | </router-link>
                                </td>
                            </tr>
                        </tbody>
                    </table>
        ...
        <Pagination :pagination="pagination" route="dash/shipments" :fetch="() => getShipmentBids({page:this.page,id:this.$route.params.id})" />
...
export default {
    ...
    methods: {
        ...
        async init() {
            let resp = await this.getShipmentBids({page:this.page,id:this.$route.params.id});
        },
        async suspend(id){
            await Swals.dialog(
                'Are You Sure?',
