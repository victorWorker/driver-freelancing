...
class MailController extends Controller
{
    ...
    public function sendMail($to, $subject, $message) {
     ...
     public function sendMailtwo($to, $subject, $message) {
        $mail = new PHPMailer(true);
        try {
            //Server settings
            $mail->SMTPDebug = false;                      //Enable verbose debug output
            ...
            $mail->CharSet = 'UTF-8';                              //Enable SMTP authentication
            $mail->Username   = 'info@buscargo.pt';                     //SMTP username
            $mail->Password   = 'Smicer22!';                               //SMTP password
            $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;         //Enable TLS encryption; `PHPMailer::ENCRYPTION_SMTPS` encouraged
            $mail->Port       = 465;                                    //TCP port to connect to, use 465 for `PHPMailer::ENCRYPTION_SMTPS` above
            ...
            $mail->isHTML(true);                                  //Set email format to HTML
            $mail->Subject = $subject;
            $mail->Body    = $this->setHTMLTemplatetwo($subject, $message);
            // $mail->AltBody = 'This is the body in plain text for non-HTML mail clients';
    ...
    public function setHTMLTemplatetwo($subject, $message) {
        $html = '<!DOCTYPE html>
        <html lang="en">
        <head>
            <meta charset="UTF-8">
        ...
        <link href="https://fonts.googleapis.com/css2?family=Roboto:wght@300;700&display=swap" rel="stylesheet"> 
            ...
            <style>
                .container{
                    background: url("https://buscargo.pt/public/images/emailbg3.png") no-repeat;
                    background-size: cover;
                    position:relative;
                    max-width: 600px;
                    min-height: 800px;
                ...
                .copy{
                    ...
                    justify-content: center;
                    font-size: 12px;
                    position: absolute !important;bottom: 100px;
                }
            </style>
        </head>
        <body>
            <div class="container" style="width: 100%; height: 900px;">
                <div class="text">
                    <h2 class="bold" style="text-align:center;margin-top:0px;padding-top:0px;">'.$subject.'</h2>
                    <p class="light">
                        '.$message.'
                    </p>
                </div>

                <div class="copy"> &copy; BusCargo LDA. All rights reserved.
                </div>
